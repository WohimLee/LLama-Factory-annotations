...
def run_exp(args: Optional[Dict[str, Any]] = None, callbacks: List["TrainerCallback"] = []):
    # 添加一个日志回调函数到回调列表中，用于记录训练日志
    callbacks.append(LogCallback())
    
    # 从传入的参数字典中提取各类训练相关的参数
    model_args, data_args, training_args, finetuning_args, generating_args = get_train_args(args)

    # 根据微调的不同阶段，选择相应的函数进行训练
    if finetuning_args.stage == "pt":
        # 如果阶段为 "pt"（预训练阶段），调用 run_pt 函数
        ...
        # 如果阶段为 "ppo"（近端策略优化阶段），调用 run_ppo 函数
        run_ppo(model_args, data_args, training_args, finetuning_args, generating_args, callbacks)
    elif finetuning_args.stage == "dpo":
        # 如果阶段为 "dpo"（特定训练阶段），调用 run_dpo 函数
        run_dpo(model_args, data_args, training_args, finetuning_args, callbacks)
    elif finetuning_args.stage == "kto":
        # 如果阶段为 "kto"（另一种特定训练阶段），调用 run_kto 函数
        run_kto(model_args, data_args, training_args, finetuning_args, callbacks)
    else:
        # 如果传入了未知的阶段，抛出异常并提示错误
